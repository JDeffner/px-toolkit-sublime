...
Managed server/runtime and Tiger installation downloads pinned archives from upstream GitHub releases and verifies their hashes. Documentation and source links may open external sites when selected. The adapter does not provide telemetry or upload mod/game contents. A custom `server_command`, `node_path`, or `tiger_path` selects software that runs with your user permissions; use executables you trust.
