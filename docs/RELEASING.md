...
3. Clean-install and upgrade the packed artifact in isolated profiles. Confirm restart/process cleanup, automatic stable-server updates, version selection and pin persistence, return to automatic updates, update opt-out and offline reuse of the cached server. Keep the minimum Sublime/LSP and bootstrap server mapping current.
